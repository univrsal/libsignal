		...
		const T &expect, bool expect2)
{
	...
	cout << "Start test with " << id << " for " << val << " against "
	     << expect << " could retrieve: " << ok << ", expect: " << expect2
	     << endl;
		 ...
		 const T &expect, bool expect2)
{
	...
	const auto &val = in.get<T>(id, &ok, def);
	if (expect2)
		assert(fabs(static_cast<double>(val - expect)) <
		       FLOAT_LENIENCY);
	assert(ok == expect2);
}
...
void c_signal1_2(const signal_parameters_t *, signal_parameters_t *out)
{
	...
	if (out) {
		...
		assert(signal_parameters_set_string(out, "string",
						    "string123"));

		point_t p;
		p.x = 2;
		p.y = 3;
		assert(signal_parameters_set_data(out, "data", &p,
						  sizeof(point_t)));
	}
}
