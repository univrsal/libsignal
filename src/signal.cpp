...
void signal_manager_free(signal_manager_t *m) { delete m; }
...
void signal_parameters_free(signal_parameters_t *p) { delete p; }
                                         ...
                                         const char *id, bool *ok)
{
    ...
    if (str) return reinterpret_cast<std::string *>(str)->c_str();
    return nullptr;
}
