...
namespace signal {
	...
	public:
		virtual ~receiver() = 0;
		virtual void receive(const parameters &param = parameters(), parameters *out = nullptr) = 0;
	};
	...
	class signal {
		...
		bool add_receiver_obj(std::shared_ptr<receiver> &&r)
		{
			if (!r)
				return false;
			if (std::find(m_receiver_objects.begin(), m_receiver_objects.end(), r) == m_receiver_objects.end()) {
				m_receiver_objects.emplace_back(r);
				return true;
			}
			return false;
